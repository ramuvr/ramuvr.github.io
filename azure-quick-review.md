...
If you would like to run the scan periodically without manual intervention, set up a DevOps pipeline and uploads the scan results to the repository.

## Azure DevOps Project

1. Create a new Azure DevOps Project.
2. Under Project Settings Overview, you can turn off Boards, Test Plans and Artifacts as you won't need them for this project.
3. Grant permissions for Build service to write files into repository. Go to Settings, repositories and under Security tab, select the User : `{ProjectName}} Build Service (DevOpsOrgName)` and set Contribute to Allow. (no save button available)
...
There are a few options to create the service connection, but we will create a Microsoft Entra ID workload identity federation to manage a service principal which eliminates the need for you to manage secrets or secret expiration.
...
Next we need to assign the service principal permission on the Azure tenant to be able to scan all subscriptions. Select a scope, say Management group level, such that it covers all subscripts you want to scan using Azure Quick review. Assign the service principal created above to get `Reader` role permissions on the target management group scope.
...
1. Return to the tab with the "New Azure service connection" workflow in Azure DevOps and pick up where you left off.
...
4. On the 'Configure your pipeline' screen, you can chose Starter Pipeline. If you prefer to download the below pipeline and store it in advance, you can simply open the existing YAML file at this stage.
5. I have written the below pipeline to run Azurre Quick Review. You can copy the code, made adjustments and Save the YAML once done (don't hit `Save and run` yet, there is one more step to complete).
6. Notes:
   ...
   3. *I have stored the pipeline file in a folder named `.azuredevops`, use appropriate folder structure for your repository.*
...
7. Create a Variable Group: You'll notice that I have stored the service connection name in a varibale group `VG-AzureQuickReview`
    ...
    - Enter the name and value for each variable to include in the group, choosing + Add for each one. If you want to encrypt and securely store the value, choose the "lock" icon at the end of the row. In this project, we createa variable for the Service Connection and enter the name of the service connection name.
